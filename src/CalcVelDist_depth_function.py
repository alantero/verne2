...
def calcVelDist_full(m_x, sigma_p, loc, interaction, depth_in = 0):
    ...
    if (interaction not in  ["SI", "SD", "Millicharge", "hm", "ulm"]):
    ...
    def getVelDist(gamma, depth):
        ...
        for i in tqdm(range(len(vlist))):
            ...
            #One Scatter correction
            f_scatter[i] = verne.CalcF_reflected(vlist[i], gamma, depth, sigma_p, m_x, target, vfinal_interp, interaction=interaction)
    
        #Add on the final point
        vlist = np.append(vlist, vmax)
        f_final = np.append(f_final, 0.0)

        f_scatter = np.append(f_scatter, 0.0)
    
        #frac_reflected = calc_frac(sigma_p, gamma)
        return vlist, f_final, f_scatter#f_final*(1 + frac_reflected)
    ...
    gamma_list[0] = 1e-3
    gamma_list[-1] = 1 - 1e-3
    ### Array over the lenght of the Earth Radius
    depth_list = np.linspace(1e-1,6371e3,N_depth)
    vgrid = np.zeros((N_depth, N_gamma, Nv))
    fgrid = np.zeros((N_depth, N_gamma, Nv))
    fgrid_withrefl = np.zeros((N_depth, N_gamma, Nv))

    r_txt = np.zeros((N_depth, N_gamma, Nv))
    gamma_txt = np.zeros((N_depth, N_gamma, Nv))

    for r_i in range(N_depth):
        for j in range(N_gamma):
            print(">Calculating for gamma/pi = ", gamma_list[j],"and r_i = ", depth_list[r_i],"...")
            vgrid[r_i,j,:], fgrid[r_i,j,:], fgrid_withrefl[r_i,j,:] = getVelDist(gamma_list[j]*np.pi, depth_list[r_i])
            r_txt[r_i,j,:] = np.repeat(depth_list[r_i], Nv)
            gamma_txt[r_i,j,:] = np.repeat(gamma_list[j], Nv)
    ...
    fname = results_dir + "veldists/f_" + interaction + "_" + loc + "_mx" + '{0:.3f}'.format(m_x) + "_lsig" + '{0:.2f}'.format(np.log10(sigma_p)) + "_rdepth.txt"
    headertxt = "mx [GeV]: " + str(m_x) + "\nsigma_p [cm^2]: " + str(sigma_p) #+ "\ndepth [m]: " + str(depth) + "\nloc: " + target
    headertxt += "\nColumns: depth [m], gamma/pi, v [km/s], f(v, gamma) [s/km], f[with reflection](v, gamma) [s/km]"
    ...
    outdata = np.transpose([r_txt.flatten(), gamma_txt.flatten(), vgrid.flatten(), fgrid.flatten(), fgrid_withrefl.flatten()])
    try:
