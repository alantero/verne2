...
def FFcorrection_integrand(x, v, m_x, A0, interaction="SI"):
    if (interaction.lower() == "SI".lower() or interaction.lower() == "hm" or interaction.lower() =="ulm"):
...
def CalcF(vf, gamma, depth,sigma_p, m_x, target, vmax_interp, interaction="SI"):
    ...
    for i in range(len(tlist)):
        ...
        if False:#(vmax_interp(tlist[i]) < vf): 
            fint[i] = 0.0
        else:
            ...
            expL1 = np.exp(-D_over_lmbda_avg)
            P2_L1 = D_over_lmbda_avg/2*expL1 - expL1/4 + expL1**3/4
            P0_L1 = expL1
 
            #print("--- One value over one lambda time %s seconds ---" % (time.time() - start_time))
            fint[i] = f_integrand_full(vf, tlist[i], gamma, depth, sigma_p, m_x, interaction, target)*(P0_L1 + P2_L1)#*correction
...
def CalcF_reflected(vf, gamma, depth,sigma_p, m_x, target, vmax_interp, interaction="SI"):
    ...
    for i in range(len(tlist)):
        ...
        if False:#(vmax_interp(tlist[i]) < vf):
            ...
            ### Scattered fraction correction
 
            ### Path after the detector 
            #Propagate across the atmosphere
            if (target in ["atmos", "full", "no_shield", "SUF", "MPI", "EDE", "MOD", "surface"]):
                d1 = 0
            ...
            r_vec = radius(np.geomspace(d1,d2,100), tlist[i], depth)
            inv_lmbda_avg_r = inv_mean_free_path_avg_r(r_vec, params + [vf])
            L2_over_lmbda_avg = simps(inv_lmbda_avg_r, np.geomspace(d1,d2,100))
            ...
            r_vec = radius(np.geomspace(d1,d2,100), tlist[i], depth)
            inv_lmbda_avg_r = inv_mean_free_path_avg_r(r_vec, params + [vf])
            L1_over_lmbda_avg = simps(inv_lmbda_avg_r, np.geomspace(d1,d2,100))#/(d2-d1)
            expL1 = np.exp(-L1_over_lmbda_avg)
            P2_L1 = L1_over_lmbda_avg/2*expL1 - expL1/4 + expL1**3/4
            P0_L1 = expL1
 
            f_full = f_integrand_full(vf, tlist[i], gamma, depth, sigma_p, m_x, interaction, target)

            fint_2order[i] = f_full*(1-P0_L2-P2_L2)*(P0_L1+P2_L1)
    
    #Integrate with Simpson's rule
    return simps(fint_2order, tlist) 
...
def CalcF_reflected(vf, gamma, depth,sigma_p, m_x, target, vmax_interp, interaction="SI"):
    ...
    for i in range(len(tlist)):
        ...
        if (vmax_interp(tlist[i]) < vf):
            ...
            if (target in ["atmos", "full", "no_shield", "SUF", "MPI", "EDE", "MOD", "surface"]):
                d1_before = 0
                d2_before = pathLength(depth, tlist[i]) - pathLength_Earth(depth, tlist[i])

            #Propagate from the surface of the Earth to the detector
            if (target in ["earth", "full", "no_shield", "SUF", "MPI", "EDE", "MOD", "surface"]):
                d1_before = pathLength(depth, tlist[i]) - pathLength_Earth(depth, tlist[i])
                d2_before = pathLength(depth, tlist[i])
 
            params = [sigma_p, m_x, interaction, target]
            r_vec_before = radius(np.geomspace(d1_before,d2_before,100), tlist[i], depth)
            inv_lmbda_avg_r_before = inv_mean_free_path_avg_r(r_vec_before, params + [vf])
            L_over_lmbda_avg = simps(inv_lmbda_avg_r_before, np.geomspace(d1_before,d2_before,100))
            _expL = np.exp(-L_over_lmbda_avg)
            expL = np.exp(L_over_lmbda_avg)
            #correction_L1 = np.exp(-D_over_lmbda_avg)
            ### Path after the detector 
            #Propagate across the atmosphere
            if (target in ["atmos", "full", "no_shield", "SUF", "MPI", "EDE", "MOD", "surface"]):
                d1 = 0
            ...
            inv_lmbda_avg_r = inv_mean_free_path_avg_r(r_vec, params + [vf])
            L2_over_lmbda_avg = simps(inv_lmbda_avg_r, np.geomspace(d1,d2,100))
            #correction_L2 = 1 - np.exp(-D_over_lmbda_avg)
            exp2L2 = np.exp(-2*L2_over_lmbda_avg)
            
            correction_1sc = _expL/2*(1-exp2L2)
            #print(np.exp(-L_over_lmbda_avg)*(1 - np.exp(-L2_over_lmbda_avg)), correction_1sc)
            correction_2sc= L_over_lmbda_avg/2*_expL - _expL/4 + _expL**3/4
            
            #if correction_2sc > 0.05:
            #    print(correction_1sc, correction_2sc)

            #fint[i] = f_integrand_full(vf, tlist[i], gamma, depth, sigma_p, m_x, interaction, target)*(1 - np.exp(-L2_over_lmbda_avg))#correction_1sc#correction_L1*correction_L2
            print(f_integrand_full(vf, tlist[i], gamma, depth, sigma_p, m_x, interaction, target)) 
            fint[i] = f_integrand_full(vf, tlist[i], gamma, depth, sigma_p, m_x, interaction, target)#*correction_L1*correction_L2
            fint_2order[i] = f_integrand_full(vf, tlist[i], gamma, depth, sigma_p, m_x, interaction, target)*correction_2sc
    #Integrate with Simpson's rule
    return simps(fint, tlist) #+ simps(fint_2order, tlist)# + np.pi)
...
def sigma_correction_light(Emax,Z,A):
    #if (NEGLECT_FF):
    #    return 1.0
    m_A = A*931.5*1e3 # keV
    Emax = Emax # keV
    ...
    #print(old)
    #print(a**4*qref**4/(1+aq**2))

    return 0.5*a**4*qref**4/(1+aq**2)


def sigma_correction_heavy(Emax,Z,A):
    #if (NEGLECT_FF):
    #    return 1.0
    m_A = A*931.5*1e3 # keV
    Emax = Emax # keV
    ...
    a = 1/4*(9*np.pi**2/(2*Z))**(1/3)*a0#/keVfm_1
    aq = a*q
    return 7/8*( 1 + 1/(1+aq**2) - 2/aq**2*np.log(1+aq**2) )

def sigma_correction(Emax,Z,A,interaction):
    if interaction.lower()=="hm":
        return sigma_correction_heavy(Emax,Z,A)
    else:
        return sigma_correction_light(Emax,Z,A)
...
def inv_mean_free_path_avg_r(r, params):
    ...
    for i in isovals:
        ...
        if (interaction.lower() == "hm" or interaction.lower() =="ulm"):
            inv_mean_free_path_avg += inv_mean_free_path_vec(sigma_p, m_x, n_i, C[i], mediator=[v,Avals[i]],interaction=interaction)
        else:
...
def dv_by_dD(v, D, params):
    ...
    for i in isovals:
        ...
        if (interaction.lower() in ["SI".lower(), "hm", "ulm", "Millicharge".lower()]):
